  ...
  methods: {
    ...
    drawBorder() {
      ...
      this.context.translate(
        this.canvas.width - SVG_CORNER.viewBox.width,
        this.canvas.height - SVG_CORNER.viewBox.height,
      )
      ...
      this.context.rect(
        SVG_CORNER.viewBox.width,
        0,
        this.canvas.width - 2 * SVG_CORNER.viewBox.width,
        this.canvas.height,
      )
      this.context.clip()
      for (let tile = 0; tile < Math.ceil(this.canvas.width / SVG_TOP.viewBox.width); tile += 1) {
        ...
        this.context.save()
        this.context.translate(
          tile * SVG_TOP.viewBox.width,
          this.canvas.height - SVG_TOP.viewBox.height,
        )
      ...
      this.context.rect(
        0,
        SVG_CORNER.viewBox.height,
        this.canvas.width,
        this.canvas.height - 2 * SVG_CORNER.viewBox.height,
      )
      this.context.clip()
      for (let tile = 0; tile < Math.ceil(this.canvas.height / SVG_TOP.viewBox.width); tile += 1) {
        ...
        this.context.save()
        this.context.translate(
          this.canvas.width - SVG_TOP.viewBox.height,
          tile * SVG_TOP.viewBox.width,
        )
        this.rotate(this.context, SVG_TOP.viewBox, 90)
        // Correct origin after rotation, so that border is at top left and not center
        this.context.translate(
          -(SVG_TOP.viewBox.height - SVG_TOP.viewBox.width) / 2,
          (SVG_TOP.viewBox.width - SVG_TOP.viewBox.height) / 2,
        )
        this.context.fill(SVG_TOP.path)
        this.context.stroke(SVG_TOP.path)
        this.context.restore()
...
<template>
  <canvas ref="canvas">
    A progress bar visualizing how many points have been gained (in green) and how many are still
    open (in gray).
  </canvas>
</template>
